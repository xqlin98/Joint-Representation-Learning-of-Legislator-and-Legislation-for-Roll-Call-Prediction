...
def prediction_on_test(args,best_model, test, legislator_embed):
    ...
    with torch.no_grad():
        for legislation_name1 in test:
            ...
            right += sum(np.multiply(pred_result == gt_result,y_mask))
            total += sum(y_mask)
            # find the high deviation member in the prediction result
            for high_mem in high_dev_member:
                if (high_mem in member_dict) and (y_mask[member_dict[high_mem]]==1):
                    if high_mem not in high_dev_member_pred:
                        high_dev_member_pred[high_mem] = {}
                        high_dev_member_pred[high_mem]['prediction'] = {'right':0,'total':0}
                        high_dev_member_pred[high_mem]['mem_info'] = high_dev_member[high_mem]
                    high_dev_member_pred[high_mem]['prediction']['total'] += 1
                    high_dev_member_pred[high_mem]['prediction']['right'] += int(gt_result[member_dict[high_mem]] == pred_result[member_dict[high_mem]])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--time_end', type=int, default=2018,
                    help='whether to check result')
    parser.add_argument('--time_len', type=int, default=4,
                    help='whether to check result')
    parser.add_argument('--cuda', type=int, default=0,
                    help='whether to check result')
    parser.add_argument('--result', type=str, default='deepwalk',
    ...
    with open('high_dev_member.json') as f:
        high_dev_member = json.load(f)

    high_dev_member_pred = {}
    vote_dict = {'Yea': 0, 'Aye': 0, 'Nay': 2,
                 'No': 2, 'Not Voting': 1, 'Present': 1}
    acc_test = []
    acc_val = []
    for time_end in range(args.time_start,args.time_end+1):
        if time_end in [2007,2008,2011,2012,2013]:
            continue
        ...
        embed_dict = {}
        f = open("./baseline/deepwalk/{}_embed".format(time_end))             
        line = f.readline()               
        while line:
            line = f.readline()
            embed = line.strip('\n').split(' ')
        ...
        with open('high_dev_{}.json'.format(args.result),'w') as f:
            json.dump(high_dev_member_pred, f)
    acc_val = np.array(acc_val)
    acc_test = np.array(acc_test)
    result = pd.DataFrame({'test acc':acc_test,'val acc':acc_val})
    result.to_csv('{}.csv'.format(args.result))
    print('DEEPWALK Final acc: %.6f' % np.mean(acc_test))