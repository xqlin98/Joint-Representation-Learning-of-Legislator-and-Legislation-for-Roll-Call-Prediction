    ...
    for epoch in range(args.epochs):
        ...
        for legislation_name in train_name_list:
            ...
            if count % args.eval == 1:
                ...
                acc_now = sum(pred_result == gt_result)/len(member_mask)
                if best_acc_legislation_acc<=acc_now:
                    acc_label = gt_result
                    label_freq = {0:0,1:0,2:0}
                    for i in range(len(gt_result)):
                        label_freq[acc_label[i]] += 1
                    if label_freq[0]/len(gt_result)<=0.8:
                        best_acc_legislation_acc = acc_now
                        best_acc_legislation = leg_rep.detach().cpu().numpy()
                        best_acc_legislator = member_rep.detach().cpu().numpy()
                        best_acc_label = gt_result
                        print('SAMPLE freq:')
                        print(label_freq)
                        np.save('leg_rep',best_acc_legislation)
                        np.save('voter_rep',best_acc_legislator)
                        np.save('label',best_acc_label)
...
def prediction_on_test(args,rep_model,leg_dict,predicted_vote_rate, gt_vote_rate_all):
    ...
    metrics = classification_report(gt_result, pred_result, target_names=target_names,output_dict=True)
    ...
    return right / total, right_gt/total, wei_recall, micro_recall, wei_f1, micro_f1
...
if __name__ == '__main__':
    ...
    acc_val = []
    acc_gt_vote_rate = []
    for time_end in range(2017,2018):
        ...
        acc_, acc_gt_vt = prediction_on_test(args,learning_model,leg_dict,predicted_vote_rate, gt_vote_rate)
    ...
    np.save('{}'.format(args.result),acc)
    np.save('{}_val'.format(args.result),acc_val)
    np.save('{}_with_gt_vote_rate'.format(args.result),acc_gt_vote_rate)
