...
class LSTM_GCN(nn.Module):
    ...
    def forward(self, sentence, member_all, state_all, party_all, mask_size, adjacent_matrix, legislation_embed):
        ...
        total_embed = torch.cat([gcn_out,out], dim=1)
    ...
    while True:
        ...
        for legislation_name in train_name_list:
            ...
            if count % 50 == 1:
                ...
                with torch.no_grad():
                    ...
                    for legislation_name1 in val_name_list:
                        ...
                        output = lstmgcn_model(legislation_input, member_input, state_input, party_input,len(member_input),adjacent_matrix, legislation_embed)
            ...
            output = lstmgcn_model(
                legislation_input, member_input, state_input, party_input, len(member_input), adjacent_matrix, legislation_embed)
...
def prediction_on_test(args,best_model, test, test_legislation_rep_dict):
    ...
    with torch.no_grad():
        for legislation_name1 in test:
            ...
            # out put from the network
            output = best_model(legislation_input, member_input, state_input, party_input,len(member_input),adjacent_matrix, legislation_embed)
